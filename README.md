...
- **🤖 Agent mode (safe first phase)** – the council creates a validated task plan, shows risks and expected results, and requires approval before each permitted step. Approved steps can use the existing calculator, date/time, web-search, and (when configured) isolated browser tools, including filling a named form field; every action appears in an audit history and the persistent Stop agent control cancels the task. Form submission remains disabled.
...
- **📈 Provider reliability telemetry** – the Settings drawer shows session success rate, failures, rate limits, average latency, and last-call latency. Providers with repeated failures are temporarily cooled down and automatically tried later; Test and Reset reliability remain available for manual recovery.
...
- **🔬 Evidence mode** – answers can be followed by an independent, clearly separated source panel showing trusted/unverified counts, independent domains, source age, and unresolved verification. Without a Tavily key, the UI explicitly says the answer is unverified instead of implying evidence.
